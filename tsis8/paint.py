...
background = pg.transform.smoothscale(pg.image.load('snake.jpg'),
                                      (800, 600))  # loading an image and overlaying it on the screen
...
class Snake:
    ...
    def add_to_snake(self):  # function for updating
        ...
        self.is_add = False
        if score % 5 == 0:
            self.speed += 10  # speed increasing
...
while running:  # starting loop
    ...
    if snake1.eat(food.x, food.y):  # function call
        ...
        if score % 5 == 0:  # condition of level up
            level += 1
