...
class Player(pg.sprite.Sprite):
    def __init__(self):
        ...
        self.rect = self.surf.get_rect(
            center=(400, 500))  # creating a rectangle superimposed on the surface of an object
    ...
    def move(self):  # machine movement function
        ...
        if keys[
            pg.K_UP] and self.rect.top > 0:  # the condition when the up button is pressed and the car is not located on the upper border of the screen
...
class Enemy(pg.sprite.Sprite):  # a class for controlling the enemies
    def __init__(self):
        ...
        self.rect = self.surf.get_rect(
            center=(random.randint(0, WIDTH - 40), -100))  # the appearance of objects outside the screen
...
enemies = pg.sprite.Group(
    [Enemy() for _ in range(4)])  # assigning a class to a variable, determining the number of objects
coins = pg.sprite.Group(
    [Coin() for _ in range(6)])  # assigning a class to a variable, determining the number of objects
